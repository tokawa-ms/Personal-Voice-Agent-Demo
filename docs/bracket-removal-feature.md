...
最適化された単一の正規表現パターンを使用:

| 正規表現パターン | 説明 |
|----------------|------|
| `/【[^】]*】\|（[^）]*）\|\[[^\]]*\]\|\([^)]*\)/g` | すべての括弧タイプを一度にマッチして削除 |
| `【[^】]*】` | 【と】の間のすべての文字を削除 |
| `（[^）]*）` | （と）の間のすべての文字を削除 |
| `\[[^\]]*\]` | [と]の間のすべての文字を削除 |
| `\([^)]*\)` | (と)の間のすべての文字を削除 |
...
4. **文法の不完全性**: 括弧内に重要な情報がある場合、削除後の文が文法的に不完全になる可能性があります
   - 例: 「これは【重要な資料】を使った説明です」→「これはを使った説明です」
   - この制限は既知の動作であり、AI Agent がプロンプト設計で括弧内を補足情報のみに限定することで緩和できます
...
4. **AI Agent プロンプトの最適化**: 
   - 括弧内には「読み上げ不要な補足情報」のみを含めるようAI Agentを設計することを推奨
   - 例: 参照文献【ファイル名.pdf】、正式名称（略称の展開）など
   - 文の主要部分を括弧で囲まないようプロンプトを調整
